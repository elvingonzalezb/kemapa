<?php
	...
	class Productos extends CI_Controller {
		...
		//////CATEGORIA(CONSUBCAT) DIRECTO A SUBCATEGORIA////////
		...
		//////CATEGORIA(SINSUBCAT) DIRECTO A PRODUCTOS ////////
		public function catProd()
		{
			...
			$dataPrincipal['titulo'] = 'producto';
			$dataPrincipal['texto'] = "";
			$dataPrincipal['cuerpo'] = 'productos_cat_view';
			$dataPrincipal['textosweb'] = $this->Generales_model->getTextosWeb($seccion);
			$dataPrincipal['nomCategoria'] =  $this->Productos_model->getNombreCategoria($id_categoria);
			$aux = $this->Productos_model->getProdxCatProd($id_categoria);
			//$aux = $this->Productos_model->getListaFilterSubCat();
			$productos = array();
		...
		public function productosSubcat()
		{
			...
			$subcategoria = $this->Productos_model->getNombreSubCategoria($id_subcategoria);
			$dataPrincipal['nomSubCategoria'] = $subcategoria;
			$dataPrincipal['id_categoria_current'] = $subcategoria->id_categoria;
	    ...
	    public function detallesProductos() {
			...
			$dataPrincipal['id_categoria_current'] = $prodactual->id_categoria;
		...
		public function nuevosProductos()
		{
			...
			$dataPrincipal = array();
			$seccion = 'nuevosproductos';
			$contenido = $this->Generales_model->getSeccion('productos');
			$dataPrincipal['bannerslateral'] = $this->Inicio_model->getBannersLateral();
			$dataPrincipal['categorias'] = $this->Inicio_model->getCategorias(6);
			$dataPrincipal['titulo'] = $contenido->titulo;
			$dataPrincipal['texto'] = $contenido->texto;
			$dataPrincipal['cuerpo'] = 'nuevos_productos_view';
			$dataPrincipal['textosweb'] = $this->Generales_model->getTextosWeb($seccion);



			/*********************** BEGIN PAGINACION NUEVOS PRODUCTOS ***************/
			/*********************** BEGIN PAGINACION NUEVOS PRODUCTOS ***************/
			/*********************** BEGIN PAGINACION NUEVOS PRODUCTOS ***************/

			$num_total_productos = $this->Productos_model->numTotalNuevosProductos();
			$dataPrincipal['num_total_resultados'] = $num_total_productos;
		    $config['base_url']	= base_url().'/nuevosproductos/';
		    $config['total_rows'] = $num_total_productos;
		    $config['per_page'] = getConfig('productos_x_pagina');
		    $config['uri_segment'] = 2;
		    $config['num_links'] = 5;
		    ...
		    $this->pagination->initialize($config);
		    $aux = $this->Productos_model->getNuevosProductosxPagina($inicio, $config['per_page']);
